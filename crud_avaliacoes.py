...
def get_avaliacoes():
    cursor = conf.cursor()
    cursor.execute("SELECT * FROM sergiflix.avaliaçao;")
...
def get_avaliacao(id_telespectador, cod_item):
    cursor = conf.cursor()
    cursor.execute(f"SELECT * FROM sergiglix.avaliaçao WHERE id_telespectador = {id_telespectador} AND cod_item = {cod_item};")
...
def add_avaliacao():
    ...
    if avaliacao.get('cod_item') is None or avaliacao.get('id_telespectador') is None or avaliacao.get('avaliacao') is None:
    ...
    sql = f"INSERT INTO sergiflix.avaliaçao VALUES ({avaliacao['id_telespectador']},{avaliacao['cod_item']},{avaliacao['avaliacao']});"
    print(sql)
    cur.execute(sql)
    conf.commit()
    cur.close()
...
def confere_item(pk):
    cursor = conf.cursor()
    cursor.execute(f"SELECT * FROM sergiflix.serie/filme WHERE cod_item = {pk};")
...
def confere_avalia(pk1, pk2):
    cursor = conf.cursor()
    cursor.execute(f"SELECT * FROM sergiflix.avaliaçao WHERE id_telespectador = {pk1} AND cod_item = {pk2};")
    album = cursor.fetchall()
    cursor.close()
    return not album
...
def update_avaliacao(id_telespectador,cod_item):
    ...
    sql = f"UPDATE sergiflix.avalia SET nota = {avaliacao['avaliacao']}" \
...
def delete_avaliacao(id_telespectador, cod_item):
    ...
    cursor = conf.cursor()
    cursor.execute(f"DELETE FROM sergiflix.avaliaçao WHERE id_telespectador = {id_telespectador} AND cod_item = {cod_item};")
    cursor.close()
